...
public:
	...
	void print() {
		for (int iNodeNum_i = 0; iNodeNum_i < iMaxNodeNum_; iNodeNum_i++) {
			Node iCurNode = noNode_[iNodeNum_i]; //説明変数

			string type = getType(iCurNode); //タイプの決定
			int iSiblingNodeNum = getSiblingNum(iCurNode, iNodeNum_i); //兄弟の番号
			int iChildNum = getChildNum(iCurNode); //ノードの子の数

			//出力
			cout << "node " << iNodeNum_i << ": ";
			cout << "parent = " << iCurNode.iParent_ << ", ";
			cout << "sibling = " << iSiblingNodeNum << ", ";
			cout << "degree = " << iChildNum << ", ";
			cout << "depth = " << iCurNode.iDepth_ << ", ";
			cout << "height = " << iCurNode.iHeight_ << ", ";
			cout << type << endl;
		}
	}
private:
	...
	int getSiblingNum(Node iCurN, int iCurNodeNum) {
		...
		Node iParNode = noNode_[iCurN.iParent_]; //説明変数
		//自分じゃなくてNILじゃないものが存在するなら
