...
void DFS(vector<int> rVal[], map<int, int> &uColor_m, int idOrSs) {
	stack<int> stack;
	stack.push(idOrSs);
...
void assignColor(int uNum, vector<int> rVal[], map<int, int> &uColor_m) {
...
int main() {
	int userNum;
	int relationNum;
//	int uColor_map[100000]; //<添え字, グループカラー>
//	map<int, int> uColor_m;
	map<int, int> uColor_m;
	//実際配列と変わらん
	...
	assignColor(userNum, relationVal, uColor_m);
